...
import { LayoutGrid, PlusCircle, Fingerprint, X, Settings, Users, MoreHorizontal, Trash2, Copy, EyeOff, Edit2, Bird } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-[280px] bg-[#020617] border-r border-gray-800 z-50 flex flex-col transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} onClick={() => setActiveMenuId(null)}>
        
        {/* LOGO AREA - Optimizado para pantallas estrechas */}
        <div className="p-4 border-b border-gray-800 flex items-center justify-between bg-black/20 shrink-0">
          <div className="flex items-center gap-2 overflow-hidden">
            <div className={`w-10 h-10 rounded-full bg-black border-2 flex items-center justify-center relative shrink-0 transition-all ${isLucel ? 'cursor-pointer border-primary shadow-lg shadow-primary/20' : 'border-gray-800'}`} onClick={isLucel ? onRavenClick : toggleHiddenMode}>
               <Bird className={`w-6 h-6 z-10 ${isLucel ? 'text-primary' : 'text-gray-700'}`} />
            </div>
            <div className="flex flex-col min-w-0">
              <h1 className="text-lg font-bold font-brand tracking-widest text-white leading-none truncate">LUCEL</h1>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-3 space-y-8 custom-scrollbar">
          <div>
            <h3 className="px-3 text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-4">{t.system || "SISTEMA"}</h3>
          ...
          <div>
            <div className="px-3 flex items-center justify-between mb-4">
               <h3 className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Vínculos Activos</h3>
            </div>
            <div className="space-y-1">
               {characters.filter(c => isHiddenModeActive ? true : !c.isHidden).map(char => (
                 <div key={char.id} className="relative group">
                    ...
                    {activeMenuId === char.id && (
                       <div className="absolute left-10 top-10 w-40 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl z-[60] overflow-hidden animate-fade-in">
                          <button onClick={(e) => { e.stopPropagation(); onEditCharacter(char); setActiveMenuId(null); }} className="w-full text-left px-4 py-3 text-xs hover:bg-white/10 text-white flex items-center gap-2 border-b border-gray-800 font-bold tracking-tighter uppercase"><Edit2 size={12} /> Editar</button>
                          <button onClick={(e) => { e.stopPropagation(); onDeleteCharacter(char.id); setActiveMenuId(null); }} className="w-full text-left px-4 py-3 text-xs hover:bg-red-900/20 text-red-500 flex items-center gap-2 font-bold tracking-tighter uppercase"><Trash2 size={12} /> Borrar</button>
                       </div>
                    )}
                 </div>
               ))}
        ...
        <div className="p-4 border-t border-gray-800 bg-black/40 space-y-3 shrink-0">
          ...
          <button onClick={() => { openSettings(); onClose(); }} className="w-full flex items-center justify-center gap-2 py-4 bg-gray-900 border border-gray-700 hover:border-primary rounded-xl text-xs text-gray-300 font-bold transition-all uppercase tracking-widest shadow-inner"><Settings size={16} /> Ajustes</button>
        </div>
      </aside>
    </>
  );
